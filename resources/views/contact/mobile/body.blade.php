<div class="sm:container sm:mx-auto mx-0 px-0 sm:px-4 my-8 sm:my-4 mx-auto">
    <div class="container px-4 mx-auto my-8" >
        ...
        <form class="flex flex-col items-start w-full"  method="post" action="{{ URL('en/contact-form-post') }}" id="contact-desktop-form">
            ...
            <h3 class="text-2xl font-semibold">{{__('frontend.contactFormH')}}</h3>
            <p class="mb-4 text-justify"> {{__('frontend.contactFormP')}}</p>
            <input type="hidden" name="country_code" id="country_code">
            <input type="text" name="name" id="name" placeholder="{{__('frontend.formFullName')}}" class="border p-2  w-full bg-transparent border border-white focus:outline-none focus:border-white focus:ring-white" @if($langSeg == 'ar') dir="rtl" @endif>
            <input type="text" name="name" id="phone" placeholder="{{__('frontend.formPhone')}}" class="border p-2 mt-3  w-full bg-transparent border border-white focus:outline-none focus:border-white focus:ring-white" @if($langSeg == 'ar') dir="rtl" @endif>
            <input type="email" name="email" id="email" placeholder="{{__('frontend.formEmail')}}" class="border p-2 mt-3 w-full bg-transparent border border-white focus:outline-none focus:border-white focus:ring-white mt-3" @if($langSeg == 'ar') dir="rtl" @endif>
            <textarea name="msg" id="" cols="10" rows="3" placeholder="{{__('frontend.formMsg')}}" class="border p-2 mt-3 w-full bg-transparent border border-white focus:outline-none focus:border-white focus:ring-white" @if($langSeg == 'ar') dir="rtl" @endif></textarea>
